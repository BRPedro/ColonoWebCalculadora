...
class ConteoCombinado:
    ...
    """
    def agrupamientoXvecinos(self,listapuntos,estado):
        self.listaEstado[estado]=False
        self.listacortes[estado]=[]
        while True:
        ...
        self.listacortes[estado]=listapuntos
        return
    # ****************************************************************************************************************************************************************************
    ...
    def centroidesBuscarPrimario(self, listapuntos, limite):
        largo=len(listapuntos)
        corte=largo/4 #para calcular el tamaño en que se tienen que dividir la lista listapuntos
        if corte>=4:
            self.listaEstado=[False,False,False,False]#lista para controlar si los hilos fueron finalizados
            t1 = threading.Thread(target=self.centroidesBuscarSecundario, args=(listapuntos[:corte], 0, limite,))
            t2 = threading.Thread(target=self.centroidesBuscarSecundario, args=(listapuntos[corte:corte * 2], 1, limite,))
            t3 = threading.Thread(target=self.centroidesBuscarSecundario, args=(listapuntos[corte * 2:corte * 3], 2, limite,))
            t4 = threading.Thread(target=self.centroidesBuscarSecundario, args=(listapuntos[corte * 3:], 3, limite,))
            t1.start()
            t2.start()
            t3.start()
            t4.start()
            while True:# Espera de que se termine todos los hilos
                if self.listaEstado[0] and self.listaEstado[1]and self.listaEstado[2]and self.listaEstado[3]:
                    return

        self.centroidesBuscarSecundario(listapuntos, 0, limite)
        while True:
            if self.listaEstado[0]:
                return
        return
    # ****************************************************************************************************************************************************************************
    ...
    """
    def procentajeVerde(self,rojo,verde,azul,limite):
        total=float(rojo)+float(verde)+float(azul)
    ...
    def agrupamientoXvecinosInicial(self,listapuntos):
        tamanno = len(listapuntos)
        corte = tamanno / 4
        t1 = threading.Thread(target=self.agrupamientoXvecinos, args=(listapuntos[:corte], 0,))
        t2 = threading.Thread(target=self.agrupamientoXvecinos, args=(listapuntos[corte:corte * 2], 1,))
        t3 = threading.Thread(target=self.agrupamientoXvecinos, args=(listapuntos[corte * 2:corte * 3], 2,))
        t4 = threading.Thread(target=self.agrupamientoXvecinos, args=(listapuntos[corte * 3:], 3,))
        t1.start()
        t2.start()
        t3.start()
        t4.start()
        ...
        while True:
            if ver[0] and ver[1]:
                break
            if ver[0] == False:
                if self.listaEstado[0] and self.listaEstado[1]:
                    ver[0] = True
                    listat1 = self.listacortes[0] + self.listacortes[1]
                    t5 = threading.Thread(target=self.agrupamientoXvecinos, args=(listat1, 0,))
                    t5.start()
            if ver[1] == False:
                if self.listaEstado[2] and self.listaEstado[3]:
                    listat2 = self.listacortes[2] + self.listacortes[3]
                    ver[1] = True
                    t6 = threading.Thread(target=self.agrupamientoXvecinos, args=(listat2, 1,))
                    t6.start()
        while True:
            if self.listaEstado[0] and self.listaEstado[1]:
                break

        return self.agrupamientoXvecinosFinal(self.listacortes[0] + self.listacortes[1])
    # ****************************************************************************************************************************************************************************
